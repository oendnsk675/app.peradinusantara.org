...
<div class="container-fluid">
    <div class="card shadow mb-4">
        <div id="filterData" class="collapse mt-4 container-fluid" aria-labelledby="headingPages" data-parent="#accordionSidebar">
            <form class="user" method="post" action="<?= base_url('P/Admin/process_exam')?>">
        ...
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            ...
            <?php if($datefrom != ''){?>
            <button class="btn btn-primary" data-toggle="modal" data-target="#modalJadwal" >Lakukan Sync Data Ujian</button>
            <?php }?>
                ...
                <table class="" border="1px" id="tableData" cellpadding="0px" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= $lr['nama_kelas'];?></td>
                            <td>
                                <input type="checkbox" class="item" name="item" value="<?= $lr['id_user'];?>" style="width:20px; height:20px" onclick="toggleCheckboxesOnly(this)">
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
...
<script>
  ...
  function exportTableToCSV(filename) {
    ...
    function requestToDB(id_users, id_kelas)
    {
        $.ajax({
          type: "GET", 
          url: "<?php echo base_url('P/Admin/process_sync_data_exam')?>",
          cache: false,
          data:  {
            ...
            if(data.status_code == 200){
                ...
                $(document).ready(function(){
                     Swal.fire({
                        ...
                        window.location.href = '<?php echo base_url('P/Admin/process_exam')?>';
                     });
                });
            }
          }             
    ...
    function getDataKelasFromAPKExam() {
        $.ajax({
            type: "GET", 
            url: "<?=$urlAPIExam;?>Api/getDataDatabaseJSON/m_course",
            cache: false,
            dataType: "JSON",
            success: function(data) {
                
                // Start creating the select element
                var selectHtml = '<select name="status" id="status" class="form-control">';
                
                // Optionally, add the default option
                selectHtml += '<option value="">--Pilih Ujian Kelas--</option>';
                
                // Loop through the JSON data and create an <option> for each course
                data.forEach(function(item) {
                    selectHtml += `<option value="${item.id_course}">${item.course_name}</option>`;
                });
                
                // Close the select tag
                selectHtml += '</select>';
                
                // Append the select element to a specific container in the HTML
